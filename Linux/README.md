...
Let's take an example of **`USB Disk`** being used in the system.
...
Use **`dmesg`** display messages from the area of kernel called **`Ring Buffer`**. When a Linux operating system boots up there were numerous messages generated by the kernel that appear on the display screen. These messages also contain logs from the hardware devices that the kernel detects and provides good indication whether it is able to configure.
...
![boot-sequence](https://github.com/Mohsem35/DevOps/assets/58659448/bc5c767b-b81b-4f5c-82b6-d91e43a74b5e)

##### BIOS POST

In this stage, BIOS runs a **`POST(Power On Self Test)`** test, to ensure the hardware components that are attached to the device are functioning correctly, if POST fails the computer may not be operable and the system will not be proceed to next stage of the boot process
